 ...
 * @LastEditTime: 2021-04-27 18:06:51
...
void ApplicationFunctionSet::ApplicationFunctionSet_SensorDataUpdate(void)
{
  { /*Battery voltage status update*/
    ...
    if (millis() - VoltageData_time > 10) //read and update the data per 10ms
    {
      ...
      {
        VoltageData_number++;
        if (VoltageData_number == 250) //Continuity to judge the latest voltage value multiple
        {
...
void ApplicationFunctionSet::ApplicationFunctionSet_Obstacle(void)
{
  if (Application_ConquerorCarxxx0.Functional_Mode == ObstacleAvoidance_mode)
  {
    ...
    else
    {
      ...
      {
        if (en != false)
        { //In order to acquire a accurate data from the ultrasonic sensor, the device should keep still.(Stop the left and right movement when the ultrasonic sensor works)
          ApplicationFunctionSet_ConquerorCarMotionControl(stop_it, 0);
        }
        AppULTRASONIC.DeviceDriverSet_ULTRASONIC_Get(&UltrasoundData_cm /*out*/);
...
#endif
      {
        if (function_xxx(UltrasoundData_cm, 0, ObstacleDetection)) //Is there an obstacle ObstacleDetection cm ahead?
        {
          //acquire timestamp
        ...
        else //If there is no obstacle within ObstacleDetection cm ahead, keep moving forward
        {
...
void ApplicationFunctionSet::ApplicationFunctionSet_Follow(void)
{
  ...
  if (Application_ConquerorCarxxx0.Functional_Mode == Follow_mode)
  {
    ...
    else
    {
      ...
      if (function_xxx(UltrasoundData_cm, 0, ObstacleDetection)) //Is there an obstacle within ObstacleDetection cm ahead?（similar to the obstacle avoidance mode）
      {
        //Keep moving forward
        ApplicationFunctionSet_ConquerorCarMotionControl(Forward, 250);
        en = false;
...
void ApplicationFunctionSet::ApplicationFunctionSet_Servo(uint8_t Set_Servo)
{
  ...
  case 1 ... 2:
  {
    ...
    if (y_angle <= 30) //minimum control
    {
      y_angle = 30;
    }
    if (y_angle >= 110) //maximum control
    {
      y_angle = 110;
    }
  ...
  case 3 ... 4:
  {
    ...
    if (z_angle <= 0) //minimum control
    {
      z_angle = 0;
    }
    if (z_angle >= 180) //maximum control
    {
      z_angle = 180;
    }
...
void ApplicationFunctionSet::CMD_CarControlTimeLimit_xxx0(uint8_t is_CarDirection, uint8_t is_CarSpeed, uint32_t is_Timer)
{
  ...
  if (Application_ConquerorCarxxx0.Functional_Mode == CMD_CarControl_TimeLimit) //enter time-limited control mode
  {
    CarControl = true;
    if (is_Timer != 0) //#1 if the pre-set time is not ... (zero)
    {
      if ((millis() - Application_ConquerorCarxxx0.CMD_CarControl_Millis) > (is_Timer)) //check the timestamp
      {
        CarControl_TE = true;
        ApplicationFunctionSet_ConquerorCarMotionControl(stop_it, 0);

        Application_ConquerorCarxxx0.Functional_Mode = CMD_Programming_mode; /*set mode to programming mode<Waiting for the next set of control commands>*/
...
{
  static boolean CarControl = false;
  if (Application_ConquerorCarxxx0.Functional_Mode == CMD_CarControl_NoTimeLimit) //enter no time-limited control mode
  {
    CarControl = true;
    CMD_CarControl(is_CarDirection, is_CarSpeed);
  }
                                                               ...
                                                               uint32_t is_LightingTimer)
{
  ...
  if (Application_ConquerorCarxxx0.Functional_Mode == CMD_LightingControl_TimeLimit) //enter time-limited control mode
  {
...
void ApplicationFunctionSet::CMD_LightingControlTimeLimit_xxx0(void)
{
  ...
  {
    LightingControl = true;
    if (CMD_is_LightingTimer != 0) //#1if the pre-set time is not ... (zero)
    {
...
{
  static boolean LightingControl = false;
  if (Application_ConquerorCarxxx0.Functional_Mode == CMD_LightingControl_NoTimeLimit) //Enter Lighting Control mode without time-limited
  {
    LightingControl = true;
    CMD_Lighting(is_LightingSequence, is_LightingColorValue_R, is_LightingColorValue_G, is_LightingColorValue_B);
  }
...
{
  if (Application_ConquerorCarxxx0.Functional_Mode == CMD_ClearAllFunctions_Standby_mode) //Command:N100 Clear all functions to enter standby mode
  {
    ApplicationFunctionSet_ConquerorCarMotionControl(stop_it, 0);
    FastLED.clear(true);
  ...
  if (Application_ConquerorCarxxx0.Functional_Mode == CMD_ClearAllFunctions_Programming_mode) //Command:N110 Clear all functions and enter programming mode
  {
    ApplicationFunctionSet_ConquerorCarMotionControl(stop_it, 0);
    FastLED.clear(true);
    AppRBG_LED.DeviceDriverSet_RBGLED_xxx(0 /*Duration*/, NUM_LEDS /*Traversal_Number*/, CRGB::Black);
...
void ApplicationFunctionSet::ApplicationFunctionSet_IRrecv(void)
{
  ...
  if (true == IRrecv_en)
  {
    ...
    case /* constant-expression */ 9:
      /* code */ if (Application_ConquerorCarxxx0.Functional_Mode == TraceBased_mode) //Adjust the threshold of the line tracking module to adapt the actual environment
      {
...
void ApplicationFunctionSet::ApplicationFunctionSet_SerialPortDataAnalysis(void)
{
  ...
  if (c == '}') //Data frame tail check
  {
    ...
    else if (!error) //Check if the deserialization is successful
    {
      ...
      case 2:                                                                    /*<Command：N 2> */
        Application_ConquerorCarxxx0.Functional_Mode = CMD_CarControl_TimeLimit; /*Car movement direction and speed control：Time limited mode*/
      ...
      case 3:                                                                      /*<Command：N 3> */
        Application_ConquerorCarxxx0.Functional_Mode = CMD_CarControl_NoTimeLimit; /*Car movement direction and speed control：No time limited mode*/
        CMD_is_CarDirection = doc["D1"];
        CMD_is_CarSpeed = doc["D2"];
#if _is_print
        Serial.print('{' + CommandSerialNumber + "_ok}");
      ...
      case 4:                                                                  /*<Command：N 4> */
        Application_ConquerorCarxxx0.Functional_Mode = CMD_MotorControl_Speed; /*motor control:Control motor speed mode*/
      ...
      case 5:                                                            /*<Command：N 5> */
        Application_ConquerorCarxxx0.Functional_Mode = CMD_ServoControl; /*servo motor control*/
        CMD_is_Servo = doc["D1"];
      ...
      case 7:                                                                         /*<Command：N 7> */
        Application_ConquerorCarxxx0.Functional_Mode = CMD_LightingControl_TimeLimit; /*Lighting control:Time limited mode*/
      ...
      case 8:                                                                           /*<Command：N 8> */
        Application_ConquerorCarxxx0.Functional_Mode = CMD_LightingControl_NoTimeLimit; /*Lighting control:No time limited mode*/
      ...
      case 110:                                                                                /*<Command：N 110>*/
        Application_ConquerorCarxxx0.Functional_Mode = CMD_ClearAllFunctions_Programming_mode; /*Clear all function:Enter programming mode*/
      ...
      case 100:                                                                            /*<Command：N 100> */
        Application_ConquerorCarxxx0.Functional_Mode = CMD_ClearAllFunctions_Standby_mode; /*Clear all function:Enter standby mode*/
#if _is_print
        Serial.print("{ok}");
        //Serial.print('{' + CommandSerialNumber + "_ok}");
#endif
      ...
      case 106: /*<Command：N 106> :/*Servo motor control*/
      {
